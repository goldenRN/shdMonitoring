...
const ClassTable = ({ limit, title }: ClassTableProps) => {
 ...
 const deleteClass=async(id: number) => {
    try {
      ...
      const data = await res.json();
      setClasss((prevData) => prevData.filter(pst => pst.c_id !== id));
      if (!res.ok) {
    ...
    <div >
      ...
      <Table>
        ...
        <TableBody>
          ...
          Classs.map((Class) => (
            <TableRow key={Class.c_id}>
              <TableCell>{Class.c_name}</TableCell>
              <TableCell>
                <div className='flex justify'><div>
                  <Link href={`/admin/Class/edit/${Class.c_id}?b_name=${Class.c_name}`}>
                  ...
                  <div className='ml-5'>
                    <button className="bg-red-200 hover:bg-red-500 text-white font-bold py-2 px-4 rounded text-xs"
                      onClick={async () => {
                        const confirmed = window.confirm(`Та "${Class.c_name}" салбар-г устгахдаа итгэлтэй байна уу?`);
                        if (!confirmed) return;
                        deleteClass(Class.c_id)
                      }}>
