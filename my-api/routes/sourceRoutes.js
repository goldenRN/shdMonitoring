...
router.post('/create', async (req, res) => {
    ...
    try {
        ...
        const existing = await pool.query(
            'SELECT * FROM source WHERE LOWER(b_name) = LOWER($1)',
            [b_name]
        );
...
router.put('/edit', async (req, res) => {
    ...
    try {
        ...
        // 2. Шинэчлэх
        const result = await pool.query(
            'UPDATE source SET sc_name, sc_status= $1, $2 WHERE sc_id = $3 RETURNING *',
            [sc_name, sc_status,sc_id ]
        );
        ...
        await pool.query(
            'UPDATE news SET source = $1 WHERE source_id = $2 RETURNING *',
            [sc_name, sc_id]
        );
